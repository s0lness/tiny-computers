/*
 * stubapps: eight do-nothing apps, so the menu's geometry can be SEEN at
 ...
 * geometry from a script that re-implements menu.c's layout would be
 * exactly the instrument decision 0010 warns about: it would read one link
 * upstream of the code being judged, and it would agree with itself.
 *
 ...
 * runtime_core.c appends (MENU_STUB_APPS - 4) of these to the four real
 * apps. That spelling was chosen because the number the reader cares about
 * is "twelve apps", and a build flag that says 8 while the screenshot is
 ...
 * (under the same #if) lends any app past the fourth one of the four real
 * icons, cycling, so a twelve-app capture shows the layout under real ink
 * instead of a grid of holes. Ink is what the layout has to be judged
 * against.
 */
#if MENU_STUB_APPS
...
const app_t g_stubApps[8] = {
    STUB_APP("stub1"), STUB_APP("stub2"), STUB_APP("stub3"), STUB_APP("stub4"),
    STUB_APP("stub5"), STUB_APP("stub6"), STUB_APP("stub7"), STUB_APP("stub8"),
};
