...
**A bilinear resample is not a redrawn icon.** It preserves every
proportion in the source faithfully (this is what let `haloProbe`'s test
offset scale by the same ratio rather than needing its own re-tuning), but
it does soften an edge slightly compared to drawing that same edge natively
at the larger size - the cost named plainly in the header comment above
`menu_icon_capture()`. Not visible at the modest scale factors this device
actually uses today (96 to 128, 1.33x) in `preview/menu-grid-5.png`, judged
by eye against the rendered PNG rather than assumed; would be worth
re-checking if a future roster cut ever pushed the scale factor much
higher.
...
- **Five apps now reads as a considered screen, not a half-empty one.** The
  ...
  claimed fixed: it is smaller, not gone.
