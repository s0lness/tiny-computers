...
export const CLEAN_INPUT_BASELINE: Record<string, string> = {
  ...
  "feature-tables-calibration.ts":
    "the assertions are about EXACT recovered values - each of the 100 synthetic taps lands a KNOWN, FIXED, per-key offset below its own prompted key's drawn centre, and every key's printed spread must be exactly 0 with its median exactly that offset, after which all ten learned centres must round-trip through three storage records and classify their own key back. A simulated controller's dropouts and 80-250px jitter would themselves perturb every one of those numbers, which is precisely what the MEDIAN-of-ten-taps exists to survive - contaminating the exact-recovery check this file is about, not testing something new. Calibration's commit path IS the numpad's own tables_gesture_tick() (tables.c: the same ARM_SAMPLES/ARM_MS/COMMIT_CONFIRM_MS/RELEASE_GRACE_MS gesture, shared rather than copied), so its robustness against dropouts and jitter is already the property repro-touch-dropout-tables.ts proves for that exact mechanism; this file is deliberately the clean, exact-recovery half, not a second copy of it.",
